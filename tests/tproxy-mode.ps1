...
$clientGen = Get-Content -Raw -Path (Join-Path $root 'root/etc/homeproxy/scripts/generate_client.uc')
$firewall = Get-Content -Raw -Path (Join-Path $root 'root/etc/homeproxy/scripts/firewall_post.ut')
$init = Get-Content -Raw -Path (Join-Path $root 'root/etc/init.d/homeproxy')
...
Assert-Match $firewall "th dport != @homeproxy_routing_port counter return" 'TProxy port filtering should work for TCP and UDP.'
