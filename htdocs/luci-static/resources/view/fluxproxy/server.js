...
function getServiceStatus() {
	return L.resolveDefault(callServiceList('fluxproxy'), {}).then((res) => {
		let isRunning = false;
		try {
			isRunning = res['fluxproxy']['instances']['sing-box-s']['running'];
		} catch (e) { }
		return isRunning;
	});
}
...
function renderStatus(isRunning, version) {
	...
	let renderHTML;
	if (isRunning)
		renderHTML = spanTemp.format('green', _('FluxProxy Server'), version, _('RUNNING'));
	else
		renderHTML = spanTemp.format('red', _('FluxProxy Server'), version, _('NOT RUNNING'));
...
return view.extend({
	...
	render(data) {
		...
		m = new form.Map('fluxproxy', _('FluxProxy Server'), renderIntroLink());
		...
		s = m.section(form.NamedSection, 'server', 'fluxproxy', _('Global settings'));
		...
		*/
		...
		o.onchange = function(ev, section_id, value) {
			...
			let tls_element = this.map.findElement('id', 'cbid.fluxproxy.%s.tls'.format(section_id)).firstElementChild;
		...
		o.value('/etc/fluxproxy/certs/server_publickey.pem');
		...
		o.depends({'tls': '1', 'tls_cert_path': '/etc/fluxproxy/certs/server_publickey.pem'});
		...
		o.value('/etc/fluxproxy/certs/server_privatekey.pem');
		o.depends({'tls': '1', 'tls_acme': '0', 'tls_reality': '0'});
		o.depends({'tls': '1', 'tls_acme': '0', 'tls_reality': null});
		o.depends({'tls': '1', 'tls_acme': null, 'tls_reality': '0'});
		o.depends({'tls': '1', 'tls_acme': null, 'tls_reality': null});
		...
		o.inputstyle = 'action';
		o.inputtitle = _('Upload...');
		o.depends({'tls': '1', 'tls_key_path': '/etc/fluxproxy/certs/server_privatekey.pem'});
