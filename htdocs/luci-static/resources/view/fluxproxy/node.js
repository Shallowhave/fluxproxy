...
function renderNodeSettings(section, data, features, main_node, routing_mode) {
	...
	o.onchange = function(ev, section_id, value) {
		let desc = this.map.findElement('id', 'cbid.fluxproxy.%s.transport'.format(section_id)).nextElementSibling;
		if (value === 'http')
		...
		let tls = this.map.findElement('id', 'cbid.fluxproxy.%s.tls'.format(section_id)).firstElementChild;
		if ((value === 'http' && tls.checked) || (value === 'grpc' && !features.with_grpc)) {
			this.map.findElement('id', 'cbid.fluxproxy.%s.http_idle_timeout'.format(section_id)).nextElementSibling.innerHTML =
			...
			this.map.findElement('id', 'cbid.fluxproxy.%s.http_ping_timeout'.format(section_id)).nextElementSibling.innerHTML =
	...
	o.value('/etc/fluxproxy/certs/client_ca.pem');
	...
	o.depends({'tls_self_sign': '1', 'tls_cert_path': '/etc/fluxproxy/certs/client_ca.pem'});
	...
	o.value('/etc/fluxproxy/certs/client_ech_conf.pem');
	o.depends('tls_ech', '1');
	o.modalonly = true;
	...
	o.inputstyle = 'action';
	o.inputtitle = _('Upload...');
	o.depends({'tls_ech': '1', 'tls_ech_config_path': '/etc/fluxproxy/certs/client_ech_conf.pem'});
	...
	if (features.with_utls) {
		...
		o.validate = function(section_id, value) {
			if (section_id) {
				let tls_reality = this.map.findElement('id', 'cbid.fluxproxy.%s.tls_reality'.format(section_id)).firstElementChild;
...
return view.extend({
	...
	render(data) {
		...
		m = new form.Map('fluxproxy', _('Edit nodes'));

		s = m.section(form.NamedSection, 'subscription', 'fluxproxy');
		...
		o.placeholder = 'Wget/1.21 (FluxProxy, like v2rayN)';
		...
		o.onclick = function() {
			return fs.exec_direct('/etc/fluxproxy/scripts/update_subscriptions.uc').then((res) => {
